"""Tests for the POST /cron/strength-sync route in interfaces/web_server.py.

Mirrors tests/test_web_server.py: stub heavy web_server imports, exercise the
OIDC gate (dev-bypass + 401 without bearer), and confirm run_one_batch is
...
def test_strength_sync_returns_batch_result_with_dev_bypass(monkeypatch):
    stubs = _stub_web_server_imports()
    with patch.dict(sys.modules, stubs):
        import interfaces.web_server as ws  # noqa: PLC0415
        from fastapi.testclient import TestClient  # noqa: PLC0415

        batch = {"ok": True, "mode": "delta", "processed": 3, "deleted": 0, "done": True}
        with patch.dict(os.environ, _BASE_ENV):
            with patch("core.strength_ingest.run_one_batch", return_value=batch) as rob:
                client = TestClient(ws.app, raise_server_exceptions=True)
                resp = client.post("/cron/strength-sync")
...
def test_strength_sync_returns_401_without_bearer(monkeypatch):
    stubs = _stub_web_server_imports()
    with patch.dict(sys.modules, stubs):
        import interfaces.web_server as ws  # noqa: PLC0415
        from fastapi.testclient import TestClient  # noqa: PLC0415
