"""Tests for the POST /cron/run-sync route in interfaces/web_server.py.
...
def test_run_sync_returns_batch_result_with_dev_bypass():
    ...
    with patch.dict(sys.modules, stubs):
        ...
        with patch.dict(os.environ, _BASE_ENV):
            with patch("core.run_ingest.run_one_batch", return_value=batch) as rob:
                client = TestClient(ws.app, raise_server_exceptions=True)
                resp = client.post("/cron/run-sync")
...
def test_run_sync_returns_401_without_bearer(monkeypatch):
    stubs = _stub_web_server_imports()
    with patch.dict(sys.modules, stubs):
        import interfaces.web_server as ws  # noqa: PLC0415
        from fastapi.testclient import TestClient  # noqa: PLC0415
