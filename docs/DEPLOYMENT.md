...
- **GitHub repository secrets to be configured** (you will set these in
  ...
  | `KLAUS_USER_ID` | Canonical numeric namespace shared by interactive MCP, routines, and Pinecone. During migration, this deployed environment variable derives from `TELEGRAM_ALLOWED_USER_IDS`; do not create a separate GitHub secret. Move it to a dedicated provider-neutral secret only when Telegram is removed. |
...
| records (habit_completions/{date}/records) | `habit_id` ASC — **COLLECTION_GROUP scope** | Phase 28 | Required by `HabitStore.get_history()` + `HabitStore.delete()` which run a `collection_group("records")` equality query on `habit_id` (completions live under per-date subcollections). **Collection-group single-field equality is NOT covered by the automatic single-field index** (that only covers the parent collection scope) — without it every `get_history` throws `FAILED_PRECONDITION`, which `get_history` swallows into `streak 0` + an empty contribution grid for every habit (shipped Phase 28, caught 2026-07-02 on first real use). **This is a single-field index (collection-group scope), NOT a composite** — `gcloud firestore indexes composite create` rejects it with `INVALID_ARGUMENT: this index is not necessary`, and `gcloud firestore indexes fields update` can't set `query-scope=COLLECTION_GROUP` (the `--index` flag only accepts `order`/`array-config`). Create it by opening the `create_exemption=…` link that Firestore prints in the `FAILED_PRECONDITION` log line ("You can create it here: …") and clicking **Create** — it pre-fills the records/habit_id ascending collection-group exemption. Builds in ~2–5 min; no redeploy needed. |
