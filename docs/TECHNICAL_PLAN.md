...
* **Phase 10:** Morning briefing. ✓ Complete — Garmin-sync-anchored daily briefing via Telegram. State machine in Firestore (`morning_briefings/{date}`), polled every 10 min by Cloud Scheduler (`*/10 6-10 * * *` Asia/Jerusalem). Data sources: weather, calendar, Gmail, Garmin, Things 3 snapshot. Briefing injected into conversation history as assistant turn. Manual trigger via Smart Agent tool `run_morning_briefing`. New modules: `core/morning_briefing.py`, `mcp_tools/things_snapshot.py`, `prompts/morning_briefing.md`. Mac poller extended with `push_things_snapshot()`. Cloud Scheduler job: `Klaus-morning-briefing-tick`.
...
* **Firestore database:** `Klaus-firestore`
  ...
  * Collection `things_snapshot/latest` — Phase 10 Mac-side Things 3 task snapshot
...
* **Secrets in Secret Manager:** `Klaus-anthropic-key`, `Klaus-gemini-key`, `Klaus-telegram-token`, `Klaus-telegram-webhook-secret`, `Klaus-google-oauth-token`, `Klaus-pinecone-key`, `Klaus-home-address`, `GARMIN_EMAIL`, `GARMIN_PASSWORD`, `READWISE_TOKEN`