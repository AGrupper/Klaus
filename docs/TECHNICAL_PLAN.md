...
* **Phase 11:** Notion integration. ✗ Retired (connector subtraction, 2026-08) — was `mcp_tools/notion_tool.py` (5 tools: search, get_page, query_database, create_page, append_blocks), wired in `core/tools.py` behind a static `NOTION_API_TOKEN`. Removed in favour of Claude's own Notion connector, which reaches the whole workspace rather than only integration-shared pages. Klaus persisted nothing from Notion, so no data migration was required.
