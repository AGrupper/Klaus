...
- Things owns recurrence: it spawns the next instance itself, so `complete()` returns `next_id: None` and Klaus must never create the follow-up. Klaus never hard-deletes a to-do either — `task_delete` trashes, which is recoverable. Things has never once written a `t=2` record in the account's history; every hard delete there is Klaus's, and appending one is the only way to retract a poisoned record, so `commit()` exempts deletes from id validation
- A Things item id is a **128-bit UUID rendered in Base58** — not a string of random Base58 characters. Two separate traps, both crashing Things on launch on every device via `decodeBase58String.mapBase58`: (1) the alphabet excludes `0`, `O`, `I`, `l` (2026-08-13, three outages); (2) **22 Base58 digits hold 1.835× more than 2^128, so 45% of random 22-char ids overflow the decoder** (2026-08-14, five outages). The second fires on a coin flip, so it masquerades as a bug in whatever changed most recently — it was misdiagnosed in turn as note checksums, note dialects, amending fresh items, and hard deletes, each "confirmed" by a write that happened to draw a good id. Always mint ids with `things_tool.new_uuid()`, which generates the integer and encodes it so overflow is impossible by construction; `commit()` re-checks magnitude, alphabet and note checksums as a last line of defence
- Things does **not** zero-pad ids: a UUID below 58^21 renders in 21 digits and stays there, and the live account holds several. Validate an id by its decoded *value*, never by requiring 22 characters — that mistake deleted two of Amit's real to-dos (2026-08-14, recovered from the append-only journal)
