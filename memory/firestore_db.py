...
class RoutesUsageStore:
    """Persisted Routes cache plus atomic daily and rolling-minute quotas."""

    _USAGE_COLLECTION = "routes_usage"
    _RATE_COLLECTION = "routes_rate_limits"
    _CACHE_COLLECTION = "routes_cache"
    ...
    def _normalise_now(now=None):
        """Return an aware UTC datetime for quota and expiry comparisons."""
        from datetime import datetime, timezone
    ...
    @classmethod
    def _usage_document_id(cls, user_id: str | int, now=None) -> tuple[str, str]:
        """Return canonical user/day key using the Jerusalem calendar date."""
        from zoneinfo import ZoneInfo

        instant = cls._normalise_now(now)
        day = instant.astimezone(ZoneInfo("Asia/Jerusalem")).date().isoformat()
        canonical_user = str(user_id).strip()
        if not canonical_user or "/" in canonical_user:
            raise ValueError("user_id must be a non-empty canonical identifier")
    ...
    @staticmethod
    def _departure_window(departure_time_iso: str) -> str:
        """Bucket a departure into a stable 30-minute timezone-aware window."""
        from datetime import datetime, timezone

        departure = datetime.fromisoformat(str(departure_time_iso).replace("Z", "+00:00"))
        if departure.tzinfo is None:
            departure = departure.replace(tzinfo=timezone.utc)
        window = departure.replace(
            minute=(departure.minute // 30) * 30,
            second=0,
            microsecond=0,
        )
    ...
    def _cache_document_id(
        ...
        """Return an opaque Firestore-safe key and its departure window."""
        import hashlib

        window = cls._departure_window(departure_time_iso)
        material = f"{user_id}\n{event_id}\n{window}".encode("utf-8")
        return hashlib.sha256(material).hexdigest(), window
    ...
    def reserve(
        ...
        *,
        ...
        """Atomically reserve one billable Routes computation.

        The daily counter and rolling 60-second timestamps are updated in one
        Firestore transaction. Ledger failures raise so the caller omits the
        estimate rather than bypassing either cost boundary.
        """
        from datetime import datetime, timedelta

        instant = self._normalise_now(now)
        document_id, day = self._usage_document_id(user_id, instant)
        daily_document = self._client.collection(self._USAGE_COLLECTION).document(
            document_id
        )
        rate_document = self._client.collection(self._RATE_COLLECTION).document(
            str(user_id)
        )
        transaction = self._client.transaction()
        ...
        @firestore.transactional
        def reserve_once(txn):
            daily_snapshot = daily_document.get(transaction=txn)
            daily_record = (
                daily_snapshot.to_dict() or {} if daily_snapshot.exists else {}
            )
            rate_snapshot = rate_document.get(transaction=txn)
            rate_record = rate_snapshot.to_dict() or {} if rate_snapshot.exists else {}
            daily_count = max(0, int(daily_record.get("computation_count", 0)))
            cutoff = instant - timedelta(seconds=60)
            recent = []
            for raw in rate_record.get("recent_computations", []) or []:
                try:
                    parsed = self._normalise_now(
            ...
            if daily_count >= max(1, int(daily_limit)):
                return False
            if len(recent) >= max(1, int(minute_limit)):
                return False
            ...
            txn.set(
                daily_document,
                {
                    "user_id": str(user_id),
                    "date": day,
                    "computation_count": daily_count + 1,
            ...
            txn.set(
                ...
                {
                    "user_id": str(user_id),
                    "recent_computations": [item.isoformat() for item in recent],
                    "minute_limit": max(1, int(minute_limit)),
                    "updated_at": firestore.SERVER_TIMESTAMP,
                },
                merge=True,
            )
    ...
    def get_cached(
        ...
        *,
        ...
        """Return an unexpired persisted result for this event/window."""
        instant = self._normalise_now(now)
        document_id, _window = self._cache_document_id(
            user_id, event_id, departure_time_iso
        )
        try:
            snapshot = self._client.collection(self._CACHE_COLLECTION).document(
                document_id
            ).get()
            ...
            if expires_at is None or self._normalise_now(expires_at) <= instant:
                return None
            result = record.get("result")
            return dict(result) if isinstance(result, dict) else None
        except Exception:
            logger.warning("RoutesUsageStore.get_cached() failed", exc_info=True)
            raise
    ...
    def put_cached(
        ...
        *,
        ...
        """Persist one successful route result for thirty minutes."""
        from datetime import timedelta

        instant = self._normalise_now(now)
        document_id, window = self._cache_document_id(
            user_id, event_id, departure_time_iso
        )
        self._client.collection(self._CACHE_COLLECTION).document(document_id).set(
            {
                "user_id": str(user_id),
                ...
                "result": dict(result),
                "created_at": instant,
                "expires_at": instant + timedelta(minutes=self._CACHE_TTL_MINUTES),
            }
        )

    def summary(self, user_id: str | int, *, now=None) -> dict:
        """Return public quota health without exposing Routes credentials."""
        from datetime import timedelta

        instant = self._normalise_now(now)
        document_id, day = self._usage_document_id(user_id, instant)
        try:
            snapshot = self._client.collection(self._USAGE_COLLECTION).document(
                document_id
            ).get()
            record = snapshot.to_dict() or {} if snapshot.exists else {}
            rate_snapshot = self._client.collection(self._RATE_COLLECTION).document(
                str(user_id)
            ).get()
            rate_record = rate_snapshot.to_dict() or {} if rate_snapshot.exists else {}
            recent_cutoff = instant - timedelta(seconds=60)
            recent_count = 0
            for raw in rate_record.get("recent_computations", []) or []:
                try:
                    from datetime import datetime
                ...
                if parsed > recent_cutoff:
                    recent_count += 1
            daily_count = max(0, int(record.get("computation_count", 0)))
            return {
                "date": day,
                "computation_count": daily_count,
                "daily_limit": 25,
                "daily_remaining": max(0, 25 - daily_count),
                "rolling_minute_count": recent_count,
                "minute_limit": 5,
                "minute_remaining": max(0, 5 - recent_count),
            }
        except Exception:
            logger.warning("RoutesUsageStore.summary() failed", exc_info=True)
            return {}
